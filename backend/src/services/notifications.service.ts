...
function getNotificationTag(daysBefore: number) {
...
export async function processExpiringPermitNotifications(options: PermitNotificationDeps = {}) {
  ...
  let sentCount = 0;
  let skippedCount = 0;
  const eligiblePermits = permits.filter((permit: { fechaVencimiento: Date }) => {
