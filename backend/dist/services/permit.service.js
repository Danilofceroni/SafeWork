...
import { construirContexto, esEstadoTerminal, puedeTransicionar, resolverTransicion, } from "./workflow.engine.js";
const ESTADOS_CONTRATISTA = ["PENDIENTE_CONTRATISTA", "OBSERVADO"];
// ----------------------------------------------------------------
// Generación de código: PTW-{prefijo}-{sigla}-{año}-{correlativo}
// ----------------------------------------------------------------
async function generarCodigo(tenantId, prefijoTipo, sigla, anio) {
...
// Crear permiso
// ----------------------------------------------------------------
export async function crearPermiso(input, actor) {
    ...
    if (!permitType)
        throw notFound("Tipo de permiso no encontrado");
    ...
    if (!fechaInicio)
        throw badRequest("Fecha de inicio inválida. Use YYYY-MM-DD o DD/MM/YYYY");
    const fechaVencimiento = calcularVencimiento(fechaInicio, { vigenciaDias: permitType.vigenciaDias, vigenciaHastaFinDeMes: permitType.vigenciaHastaFinDeMes }, tenant.timezone);
    ...
    return prisma.$transaction(async (tx) => {
        ...
        if (input.trabajadores && input.trabajadores.length > 0) {
            if (!companyIdCuadrilla)
                throw badRequest("No hay empresa para asociar a los trabajadores");
...
// ----------------------------------------------------------------
export async function obtenerPermiso(permitId, actor) {
    ...
    if (!permit)
        throw notFound("Permiso no encontrado");
...
export async function listarPermisos(actor, filtros = {}) {
    ...
    if (!privilegiado) {
        const or = [];
        if (actor.roles.includes("SOLICITANTE"))
            or.push({ solicitanteId: actor.userId });
        if (actor.roles.includes("CONTRATISTA") && actor.companyId)
            or.push({ companyId: actor.companyId });
        if (actor.roles.includes("PORTERIA"))
            or.push({ estado: "ACTIVO" });
...
// ----------------------------------------------------------------
export async function avanzarPermiso(permitId, actor, input = {}) {
    const permit = await prisma.permit.findFirst({
        where: { id: permitId, tenantId: actor.tenantId },
        include: {
    ...
    if (permit.estado === "OBSERVADO") {
        ...
        if (!step)
            throw badRequest(`No se puede avanzar desde el estado actual (${permit.estado})`);
...
export async function devolverPermiso(permitId, motivo, actor) {
    const permit = await prisma.permit.findFirst({
        where: { id: permitId, tenantId: actor.tenantId },
        select: { id: true, estado: true },
    });
 ...
 */
export async function cerrarPermiso(permitId, actor, observacion) {
    const permit = await prisma.permit.findFirst({
        where: { id: permitId, tenantId: actor.tenantId },
    ...
    if (!permit)
        throw notFound("Permiso no encontrado");
    if (permit.estado !== "ACTIVO")
        throw badRequest(`Solo se puede cerrar un permiso ACTIVO (está ${permit.estado})`);
 ...
 */
export async function suspenderPermiso(permitId, motivo, actor) {
    const permit = await prisma.permit.findFirst({
        where: { id: permitId, tenantId: actor.tenantId },
        select: { id: true, estado: true },
    });
    if (!permit)
        throw notFound("Permiso no encontrado");
    if (permit.estado !== "ACTIVO")
        throw badRequest(`Solo se puede suspender un permiso ACTIVO (está ${permit.estado})`);
...
export async function reactivarPermiso(permitId, actor, observacion) {
    const permit = await prisma.permit.findFirst({
        where: { id: permitId, tenantId: actor.tenantId },
        select: { id: true, estado: true },
    });
    if (!permit)
        throw notFound("Permiso no encontrado");
    if (permit.estado !== "SUSPENDIDO")
        throw badRequest(`Solo se puede reactivar un permiso SUSPENDIDO (está ${permit.estado})`);
    if (!actor.roles.includes("ADMIN") && !actor.roles.some((r) => ROLES_SUSPENSION.includes(r))) {
...
export async function rechazarPermiso(permitId, motivo, actor) {
    const permit = await prisma.permit.findFirst({
        where: { id: permitId, tenantId: actor.tenantId },
        select: { id: true, estado: true },
    });
    if (!permit)
        throw notFound("Permiso no encontrado");
    if (esEstadoTerminal(permit.estado))
        throw badRequest(`El permiso ya está en estado terminal (${permit.estado})`);
