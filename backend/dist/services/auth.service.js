...
async function cargarUsuarioConRoles(tenantId, rut) {
...
export async function login(input) {
    ...
    const payload = { sub: user.id, tenantId: tenant.id, roles, companyId: user.companyId };
    const perfil = {
        id: user.id,
        rut: user.rut,
        nombre: user.nombre,
...
export async function refrescarSesion(refreshToken) {
    ...
    const roles = user.roles.map((r) => r.role.codigo);
    const payload = { sub: user.id, tenantId: datos.tenantId, roles, companyId: user.companyId };
    return { accessToken: firmarAccessToken(payload) };
}
export async function obtenerPerfil(userId, tenantId) {
